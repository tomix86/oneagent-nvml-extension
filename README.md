...
![Host metrics display](docs/gpu_metrics.png)
\
_Fig 1. Host metrics reported by the plugin_

![PGI metrics display](docs/gpu_metrics.png)
\
...
* _High GPU utilization_ alert - raised when `gpu_utilization` exceeds predefined threshold (default: 90%) in given time period, example is shown in *Figure 3*,
...
* _High GPU memory utilization_ alert - raised when `gpu_mem_percentage_used` exceeds predefined threshold (default: 90%) relative to `gpu_mem_total` in given time period, example is shown in *Figure 4*.
...
\
_Fig 3. High GPU utilization alert as seen on metrics screen_

![High GPU memory utilization alert](docs/high_gpu_memory_utilization_alert.png)
\
_Fig 4. High GPU memory utilization alert as seen on metrics screen_
...
\
_Fig 5. Problem view for High GPU memory utilization alert_
